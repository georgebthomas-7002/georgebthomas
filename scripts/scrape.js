...
// Load existing resources for merging (preserves imported data)
...
async function fetchBlogFeed(feed) {
  ...
  try {
    ...
    const resources = items.map(item => {
      ...
      // Try to extract thumbnail from content, media, or description HTML
      let thumbnail = '';
      if (item['media:thumbnail']?.$?.url) {
        ...
        if (imgMatch && imgMatch[1] && !imgMatch[1].includes('__ptq.gif')) {
...
// Helper: Filter new resources (not already in existing data)
function filterNewResources(resources) {
  const newResources = [];
  for (const resource of resources) {
    const normalizedUrl = resource.url.toLowerCase().replace(/\/$/, '');
    if (!existingUrls.has(normalizedUrl)) {
      newResources.push(resource);
      existingUrls.add(normalizedUrl); // Mark as seen
    }
  }
  return newResources;
}
...
async function scrape() {
  ...
  if (!podcastsOnly && !blogsOnly) {
    for (const channel of config.youtube_channels || []) {
      ...
      const newVideos = filterNewResources(videos);
      console.log(`   New videos: ${newVideos.length} (${videos.length - newVideos.length} duplicates skipped)`);
      newResources.push(...newVideos);
    }
  }
  ...
  if (!youtubeOnly && !blogsOnly) {
    for (const feed of config.podcast_feeds || []) {
      ...
      const newEpisodes = filterNewResources(episodes);
      console.log(`   New episodes: ${newEpisodes.length} (${episodes.length - newEpisodes.length} duplicates skipped)`);
      newResources.push(...newEpisodes);
    }
  }
  ...
  if (!youtubeOnly && !podcastsOnly) {
    for (const feed of config.blog_feeds || []) {
      ...
      const newArticles = filterNewResources(articles);
      console.log(`   New articles: ${newArticles.length} (${articles.length - newArticles.length} duplicates skipped)`);
      newResources.push(...newArticles);
    }
  }

  console.log('\n' + '='.repeat(50));
  console.log(`📊 New resources to add: ${newResources.length}`);

  // Auto-tag only new resources if not skipped
  if (!skipTagging && newResources.length > 0) {
    newResources = await autoTagResources(newResources);
  }

  // Merge with existing resources
  const allResources = [...existingData.resources, ...newResources];

  // Sort by date (newest first, items with dates first)
  ...
  console.log(`   New resources added: ${newResources.length}`);
  console.log('\n🎉 Scrape complete!\n');
}
